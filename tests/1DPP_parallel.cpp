...
//Chemical time scales
    TChem.col(j) = abs(mole_frac.array()*AtmData(iNd, j)/m_wdot.array()).matrix();
...
//Solve for diffusion terms
    if ((j > 0) && (j < nSize-1)) {
      ...
      VectorXd D_next = handleCustomMolecularDiffusion(PlanetName, gasThermo, AtmData(iPress,j+1), AtmData(iTemp,j+1), mWt);
...
//Upper boundary condition
   if(j == 0){
      ...
      if(top == "Dirichlet"){
     mole_frac = VectorXd::Zero(nsp);
     std::string l_species_list = pinput->GetOrAddString("upperboundaryMixRat", "species", "nan");
    if(l_species_list != "nan"){
    while (std::regex_search (l_species_list,m,pattern)) {
      for (auto x:m){
        std::string species_lowerboundary_MixRat = pinput->GetString("upperboundaryMixRat", x);
        species_inx = gas2->speciesIndex(x);
   ...
   VectorXd D_this = handleCustomMolecularDiffusion(PlanetName, gasThermo, AtmData(iPress,j), AtmData(iTemp,j), mWt);
   VectorXd D_next = handleCustomMolecularDiffusion(PlanetName, gasThermo, AtmData(iPress,j+1), AtmData(iTemp,j+1), mWt);
   VectorXd D_prev = handleCustomMolecularDiffusion(PlanetName, gasThermo, AtmData(iPress,j), AtmData(iTemp,j), mWt);
...
//Contribution of U_i+1
    ...
    //  std::cout << "U_i+1 complete" << std::endl;
//Check for dn/dt
   conv.col(j) = m_wdot - (B.array()*N_this*mole_frac.array()).matrix() - (C.array()*N_prev*ChemMoleFrac.col(j+1).array()).matrix() - (A.array()*N_prev*mole_frac.array()).matrix();
...
//Inverting the matrix
   ...
   Ufuture = InvertedMatrix*Upresent;
...
init_species_list = pinput->GetString("output", "species");
  while (std::regex_search (init_species_list,m,pattern)) {
    ...
    species_inx = gas->speciesIndex(x);
    VectorXd cChem = ChemConc.row(species_inx)*6.022E23*1E3*1E-6; // #/cm^-3
