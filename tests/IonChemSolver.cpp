...
MatrixXd Kflux(int inxn, int inxNext, int inxPrev, MatrixXd Sp, MatrixXd Np,
               int ns, double Kzz, double Kzz_prev, double Kzz_next, double dx1,
               double dx2) {
  ...
  diff_flux =
      -1 *
      (((((Xnext - Xn) * beta2) / dx2) - (((Xn - Xprev)) * beta1 / dx1)) / dx);
  return diff_flux;
}

// Function for Molecular, and thermodiffusion [Hu et al., 2012]
MatrixXd Dflux(int inxn, int inxNext, int inxPrev, MatrixXd Sp, MatrixXd Np,
               int ns, VectorXd D_this, VectorXd D_prev, VectorXd D_next,
               double dx1, double dx2, MatrixXd mW, double T_prev,
               double T_this, double T_next, double mWav, double ga) {
  VectorXd diff_flux(ns), Xn(ns), Xnext(ns), Xprev(ns);
  double Nn, Nnext, Nprev;
  VectorXd beta1(ns), beta2(ns), Hs_prev(ns), Hs_next(ns);
  VectorXd phi_prev(ns), phi_next(ns);
  double Hs0_prev = 2 * mWav * ga * 1e-3 /
                    (6.022e23 * 1.38e-23 *
                     (T_prev + T_this));  // scale height of atmosphere -> 1/m
  double Hs0_next = 2 * mWav * ga * 1e-3 /
                    (6.022e23 * 1.38e-23 *
                     (T_next + T_this));  // scale height of atmosphere -> 1/m
  double dTdz1 =
      2 * (T_this - T_prev) / (dx1 * (T_prev + T_this));  // Thermodiffusion
  double dTdz2 =
      2 * (T_next - T_this) / (dx2 * (T_next + T_this));  // Thermodiffusion
  double dx = (dx1 + dx2) / 2;
  VectorXd I = VectorXd::Ones(ns);
  Hs_prev = 2 * mW * ga * 1e-3 /
            (6.022e23 * 1.38e-23 *
             (T_prev + T_this));  // scale height of species -> 1/m
  Hs_next = 2 * mW * ga * 1e-3 /
            (6.022e23 * 1.38e-23 *
             (T_next + T_this));        // scale height of species -> 1/m
  Xn = Sp.col(inxn);                    // mole fraction
  Xnext = Sp.col(inxNext);              // mole fraction
  Xprev = Sp.col(inxPrev);              // mole fraction
  Nn = Np.colwise().sum()(inxn);        // concentration (kmol/m^3)
  ...
  phi_prev =
      -0.5 * (((I * Hs0_prev) - Hs_prev - (I * dTdz1)).array() * beta1.array())
                 .matrix();
  phi_next =
      -0.5 * (((I * Hs0_next) - Hs_next - (I * dTdz2)).array() * beta2.array())
                 .matrix();
  diff_flux = -1 * (((((Xnext - Xn).array() * beta2.array()).matrix() / dx2) -
                     (((Xn - Xprev).array() * beta1.array()).matrix() / dx1)) /
                    dx);
...
MatrixXd Aflux(int inxn, int inxNext, int inxPrev, MatrixXd Sp, MatrixXd Np,
               int ns, VectorXd D_this, VectorXd D_prev, VectorXd D_next,
               double dx1, double dx2, MatrixXd mW, double T_prev,
               double T_this, double T_next, double mWav, double ga, int iE,
               VectorXd alph) {
  VectorXd diff_flux(ns), Xn(ns), Xnext(ns), Xprev(ns);
  double Nn, Nnext, Nprev;
  VectorXd beta1(ns), beta2(ns), Hs_prev(ns), Hs_next(ns);
  ...
  double dNedz1 = 2 * ((Ne_this * T_this) - (Ne_prev * T_prev)) /
                  (dx1 * ((Ne_prev * T_prev) + (Ne_this * T_this)));
  double dNedz2 = 2 * ((Ne_next * Ne_next) - (Ne_this * T_this)) /
                  (dx2 * ((Ne_next * T_next) + (Ne_this * T_this)));
  ...
  phi_prev =
      -0.5 * (((I * Hs0_prev) - Hs_prev - (I * dNedz1) - ((I + alph) * dTdz1))
                  .array() *
              beta1.array())
                 .matrix();
  phi_next =
      -0.5 * (((I * Hs0_next) - Hs_next - (I * dNedz2) - ((I + alph) * dTdz2))
                  .array() *
              beta2.array())
                 .matrix();
  diff_flux = -1 * (((((Xnext - Xn).array() * beta2.array()).matrix() / dx2) -
...
int main(int argc, char** argv) {
  ...
  while (ih < nSize) {
    ...
    // std::cout << AtmData(iAlt, ih)/1E3  << " " <<
    // ion_rate.col(ih).transpose() << std::endl;
    ih++;
  }
  InFile.close();
  ...
  if (photochem == "true") {
    ...
    for (int irxn = 0; irxn < nrxn; irxn++) {
      ...
      if (photo_cross_info != "nan") {
        ...
        if (photo_cross_database == "KINETICS") {
          MatrixXd photoXsection_info =
              ReadKINETICSCrossSection(atoi(photo_cross_.c_str()));
          photo_cross_data.col(ph_inx) = InterpolateCrossSection(
              stellar_input.row(0), photoXsection_info.row(0),
              photoXsection_info.row(1));
        }
    ...
    VectorXd Qe(
        nsp);  // Charge number for filtering ions from neutrals and electron
    ...
    while (Ttot < Tmax) {
      ...
      for (int j = 0; j < nSize; j++) {
        ...
        // Setting the multiplier for each reaction involving electron impact
        // process
        std::vector<double> fwd_rates(gas_kin2->nReactions());
        //  std::cout << "Setting rates for electron impact process" <<
        //  std::endl;
        for (int ionx = 0; ionx < ion_rxn; ionx++) {
          ...
          // std::cout << AtmData(iAlt, j)/1e3 << " " <<  ion_rate(ionx, j) <<
          // std::endl;
        }
        // std::cout << "Setting rates for electron impact process done" <<
        // std::endl;
        ...
        m_wjac =
            gas_kin2
                ->netProductionRates_ddX();  // Extracting Jacobian from Cantera
        mm = gas->meanMolecularWeight();     // Mean molcular weight
        // Upper boundary flux
        if (j == 0) {
          fluxT = flux_upper;
          D_Max = handleCustomMolecularDiffusion(PlanetName, gasThermo,
                                                 AtmData(iPress, j),
                                                 AtmData(iTemp, j), mWt)
                      .maxCoeff();
        }
        ...
        if ((j > 0) && (j < nSize - 1)) {
          ...
          D_j = handleCustomMolecularDiffusion(PlanetName, gasThermo,
                                               AtmData(iPress, j),
                                               AtmData(iTemp, j), mWt);
          Keddy_prev = AtmData(iKzz, j - 1);
          D_prev = handleCustomMolecularDiffusion(PlanetName, gasThermo,
                                                  AtmData(iPress, j - 1),
                                                  AtmData(iTemp, j - 1), mWt);
          Keddy_next = AtmData(iKzz, j + 1);
          D_next = handleCustomMolecularDiffusion(PlanetName, gasThermo,
                                                  AtmData(iPress, j + 1),
                                                  AtmData(iTemp, j + 1), mWt);
          // Diffusion terms (central difference scheme)

          // Molecular diffusion
          fluxM = Dflux(j, iNext, iPrev, a, n_conc, nsp, D_j, D_prev, D_next,
                        AtmData(iAlt, j - 1) - AtmData(iAlt, j),
                        AtmData(iAlt, j) - AtmData(iAlt, j + 1), mWt,
                        AtmData(iTemp, j - 1), AtmData(iTemp, j),
                        AtmData(iTemp, j + 1), mm, g);

          // Ambipolar diffusion
          fluxA = Aflux(j, iNext, iPrev, a, n_conc, nsp, D_j, D_prev, D_next,
                        AtmData(iAlt, j - 1) - AtmData(iAlt, j),
                        AtmData(iAlt, j) - AtmData(iAlt, j + 1), mWt,
                        AtmData(iTemp, j - 1), AtmData(iTemp, j),
                        AtmData(iTemp, j + 1), mm, g, iE, alpha);

          // Eddy diffusion
          fluxK = Kflux(j, iNext, iPrev, a, n_conc, nsp, Keddy_j, Keddy_prev,
                        Keddy_next, AtmData(iAlt, j - 1) - AtmData(iAlt, j),
                        AtmData(iAlt, j) - AtmData(iAlt, j + 1));
        ...
        //Source term
          gas_kin2->getCreationRates(&Source[0]); //Extracting source term from
        Cantera
        ...
        //Loss term and normalized loss
          gas_kin2->getDestructionRates(&Loss[0]); //Extracting loss term from
        Cantera ProdRates.col(j) = Source; DiffRates.col(j) = Loss; Loss =
        (Loss.array()/n_conc.col(j).array()).matrix(); //Normalizing the loss
        term Source =  (isnan(abs(Loss.array())) ).select(0, Source); Loss =
        (isnan(abs(Loss.array())) ).select(0, Loss);
        //New concentration
          VectorXd Ix = VectorXd::Ones(nsp);
          n_conc.col(j) = (((Source*dt) + n_conc.col(j)).array()/(Ix -
        (Loss*dt)).array()).matrix(); double N_this = n_conc.colwise().sum()(j);
        ...
        */
        ...
        //    std::cout << "dt: " << t_chem << std::endl;
            ...
            DiffRates.col(j) = fluxT;
            conv.col(j) = (abs(dQ.array()/mole_frac.array())).matrix();
            conv.col(j) =  (isnan(abs(conv.col(j).array())) ).select(0,
        conv.col(j)); m_subcycle = m_subcycle + dQ; if(dt == Tmin){ conv.col(j)
        = (abs(conv.col(j).array()).isInf()).select(0, conv.col(j));
            }

        //   std::cout << "outside the update loop: " << conv.col(j).maxCoeff()
        << std::endl;
        ...
        */
        ...
        pinput->GetOrAddString("upperboundaryMixRat", "species", "nan");
            ...
            while (std::regex_search (u_species_list,m,pattern)) {
              for (auto x:m){
                std::string species_upperboundary_MixRat =
        pinput->GetString("upperboundaryMixRat", x); species_inx =
        gas->speciesIndex(x); mole_frac(species_inx) =
        atof(species_upperboundary_MixRat.c_str());
            }
  ...
  if (photochem != "true") {
    ...
    int i = 0;
    int iSource = 0;
    std::cout << "Starting the simulation now (without photochemistry ofc!)"
              << std::endl;
    while (Ttot < Tmax) {
      for (int j = 0; j < nSize; j++) {
        // Setting T, P, X for each grid point
        ...
        if ((j > 0) && (j < nSize - 1)) {
          ...
          // Diffusion terms (central difference scheme)
          flux1 = Kflux(j, iNext, iPrev, a, n_conc, nsp, Keddy_j, Keddy_prev,
                        Keddy_next, AtmData(iAlt, j - 1) - AtmData(iAlt, j),
                        AtmData(iAlt, j) - AtmData(iAlt, j + 1));
        }
        ...
        conv.col(j) =
            m_wdot - flux1;  //(dQ.array()/mole_frac.array()).matrix();
        conv.col(j) = (isnan(abs(conv.col(j).array()))).select(0, conv.col(j));
  ...
  // std::vector<Eigen::MatrixXd> matrix3d(Time.size(), Eigen::MatrixXd(nsp,
  // nSize));
  ...
  int dimids[3];
  int dWav[2], iWav;
  int alt, iPres, iTem, iKeddy, iMolDiff, iAltz, iChem, isp, iOpacity, iProd,
      iDiff, iTs;
  ...
  while (std::regex_search(init_species_list, m, pattern)) {
    for (auto x : m) {
      ...
      species_inx = gas->speciesIndex(x);
      VectorXd cChem =
          ChemConc.row(species_inx) * 1E3 * 6.022E23 * 1E-6;  // #/cm^-3
