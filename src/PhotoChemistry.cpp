...
// Calculate the photochemical reaction rate based on wavelength, cross section
// and actinic flux
double PhotoChemRate(Eigen::MatrixXd wavelengths_,
  ...
  double c = 3e8;
  Spectral_radiance =
      (Spectral_radiance.array() * wavelengths_.array().transpose() / (h * c))
          .matrix();
  MatrixXd integrd =
      (crossSection_.array() * Spectral_radiance.array()).matrix();
...
double QPhotoChemRate(Eigen::MatrixXd wavelengths_,
  ...
  Spectral_radiance =
      (Spectral_radiance.array() * wavelengths_.array().transpose() / (h * c))
          .matrix();
  MatrixXd integrd =
      (QYield.array() * crossSection_.array() * Spectral_radiance.array())
          .matrix();
  sum = (integrd.array() * d_wavelength.array()).sum();  // Bhattacharya
                                                         // (5-1-23)
...
// The output variable will contain wavelength (m) and photoionization cross
// section (m^2)
...
// The output variable will contain wavelength (m) and photodissociation cross
// section (m^2)
...
Eigen::VectorXd VenusUV(Cantera::ThermoPhase* NetworkName, double Pres,
  ...
  if (Alt > 67) {
    for (int i = 0; i < size; i++) {
      COpacity(i) =
          0.056 * 1E-3 *
          exp(((67E3 - Alt) / 3E3) - ((wav(i) - 3600E-10) / 1000E-10));
    }
  }
