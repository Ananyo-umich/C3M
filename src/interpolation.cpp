  ...
  for (int ref_inx = 0; ref_inx < ref_size; ref_inx++) {
    ...
    while (end - start > 2) {
      ...
      if ((reference_wavelength(ref_inx) >= input_wavelength(start)) &&
          (reference_wavelength(ref_inx) < input_wavelength(pos))) {
        end = pos;
      }
    ...
    value = input_cross_section(start) +
            (reference_wavelength(ref_inx) - input_wavelength(start)) *
                (input_cross_section(start) - input_cross_section(end)) /
                (input_wavelength(start) - input_wavelength(end));
